<template>
    <div class="dashboard-box">
        <div class=" title-bar ">欢迎访问黄豆芽博客系统管理中心</div>
...
<script>
    ...
    export default {
        ...
        methods:{
            getRegisterSize(){
                api.getRegisterSize().then(result=>{
                    if(result.code===20000){
                        this.size.registerSize=result.data;

                    }
                })
            },
            getArticleSize(){
                api.getArticleSize().then(result=>{
                    if(result.code===20000){
                        this.size.articleSize=result.data;
                    }
                })
            },
            getCommentSize(){
                api.getCommentSize().then(result=>{
                    if(result.code===20000){
                        this.size.commentSize=result.data;
                    }
                })
            },
            getWebSize(){
                api.getWebSize().then(result=>{
                    if(result.code===20000){
                        this.size.webSize=result.data.value;
                    }
                })
            },
            getComment(){
